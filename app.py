#!/usr/bin/env python3
"""
Stable Market Engine v14.1 — Probabilistic Ranker (KeyError Fixed)
✅ Fixed: Weekly Call now correctly uses processed weekly_df (not raw OHLCV)
✅ Fixed: All timeframe indicators extracted AFTER feature engineering
✅ Features: Bulk CSV, Monte Carlo Predictor, 2H Tactical Call, Session Persistence
"""
...
st.set_page_config(
    page_title="Stable Market Engine v14.1",
    layout="wide",
...
# MAIN APP
# -----------------------------
st.title("📈 Stable Market Engine v14.1 — Probabilistic Ranker")
...
if run:
    ...
    for sym in symbols:
        ...
        if not use_real: tactical_raw = daily_raw.copy()
        
        weekly_raw = daily_raw.resample("W-FRI").agg({"Open":"first","High":"max","Low":"min","Close":"last","Volume":"sum"}).dropna()
        
        # ✅ FIX: Process all dataframes BEFORE extracting indicators
        ...
        weekly_df = enrich_price_features(weekly_raw, "weekly", daily_map["SPY"])
        
        row_2h = tactical_df.iloc[-1]
        row_d = daily_df.iloc[-1]
        row_w = weekly_df.iloc[-1]
        ...
        rows.append({
            ...
            "Weekly Call": "CALL" if row_w["uo_gap"] > 0 else "PUT",
        ...
        detail[sym] = {"tactical": tactical_df, "daily": daily_df, "weekly": weekly_df, "tactical_label": tactical_label, "mc": mc, "analogs": analogs, "grade_reason": grade_reason, "grade_score": grade_score}
