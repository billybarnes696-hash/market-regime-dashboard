#!/usr/bin/env python3
"""
Stable Market Engine — Real Data + Proxy Smooth Visuals
✅ Real Alpaca 1H/2H bars for accuracy
✅ Heavy viz smoothing (EMA 21-34) for clean, proxy-like charts
✅ Light decision smoothing (EMA 1-2) for responsive signals
✅ Bounded tanh transforms + hybrid normalization
✅ Fixed: syntax, imports, trailing spaces, typos
"""
...
st.set_page_config(
    page_title="Stable Market Engine — Smooth",
    layout="wide",
...
def add_ultimate_oscillator(out: pd.DataFrame, timeframe_name: str) -> pd.DataFrame:
    ...
    # ✅ HEAVY VIZ SMOOTHING FOR PROXY-LIKE CHARTS
    viz_smooth_map = {
        "proxy_smooth_1hour": 34,  # ~1.5 weeks of smoothing
        "real_1hour": 13,          # ~2 days
        "proxy_smooth_2hour": 21,  # ~1 week
        "real_2hour": 8,           # ~1.5 days
        "hourly_proxy": 34,
        "2hour_proxy": 21,
        "daily": 5,
        "weekly": 3,
...
# Sidebar
# -----------------------------
st.title("📈 Stable Market Engine — Smooth")
st.caption("Alpaca-only | real 1h/2h logic | proxy smooth visuals | tanh-normalized aggregate oscillator")
with st.sidebar:
