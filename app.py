#!/usr/bin/env python3
"""
Stable Market Engine v14 — Complete Production Version
✅ Fixed: 2-Hour Proxy now uses real 30-min Alpaca data (proper resampling)
✅ Fixed: Dropdown no longer resets dashboard (st.session_state persistence)
✅ Fixed: Pandas 3.0 compatibility ("2h" not "2H")
✅ Fixed: Alpaca SIP subscription errors → auto-fallback to Yahoo Finance
✅ Features: TSI Cross Logic | Monte Carlo Predictor | Analog Matching | Grading | Bulk CSV | Options Liquidity
"""
...
st.set_page_config(
    page_title="Stable Market Engine v14",
    layout="wide",
...
def fetch_alpaca_2hour(symbol: str, months: int, key: str, secret: str, feed: str) -> pd.DataFrame:
    ...
    # Fetch 30-min data to build accurate 2H candles
    ...
    # ✅ PANDAS 3.0 FIX: Use "2h" (lowercase) and "origin" instead of offset
    resampled = clean.resample("2h", origin=pd.Timestamp("1970-01-01 09:30:00")).agg(agg).dropna(subset=["Open", "High", "Low", "Close"])
    
    if not resampled.empty: resampled.to_parquet(p)
    return resampled

@st.cache_data(ttl=1800, show_spinner=False)
def fetch_yahoo_fallback(symbol: str, timeframe: str, years: int) -> pd.DataFrame:
    """Fallback to Yahoo Finance if Alpaca fails."""
    try:
        import yfinance as yf
        period_map = {"1H": "60d", "2H": "60d", "1D": f"{max(years, 5)}y"}
        interval_map = {"1H": "1h", "2H": "1h", "1D": "1d"}
        ...
        if timeframe in ["1H", "2H"]:
            ...
            if timeframe == "2H":
                # Resample 1H to 2H
                agg = {"Open": "first", "High": "max", "Low": "min", "Close": "last", "Volume": "sum"}
                df = df.resample("2h", origin=pd.Timestamp("1970-01-01 09:30:00")).agg(agg).dropna(subset=["Open", "High", "Low", "Close"])
...
def plot_dashboard(symbol: str, hourly_df, tactical_df, daily_df, weekly_df, asof_date, tactical_label):
    fig = make_subplots(rows=5, cols=1, vertical_spacing=0.04,
        subplot_titles=[f"{symbol} Daily Price", f"{tactical_label} Oscillator", "Daily Oscillator", "Weekly Oscillator", "Overbought/Oversold"],
    ...
    for rn, frame, nm in zip([2,3,4,5], [tactical_df.tail(260), daily_df.tail(260), weekly_df.tail(160)], [tactical_label, "Daily", "Weekly"]):
...
# MAIN APP
# -----------------------------
st.title("📈 Stable Market Engine v14 — Probabilistic Ranker")
...
with st.sidebar:
    ...
    feed = st.selectbox("Data Feed", ["iex", "sip"], index=0)
    
    st.header("Input")
    symbols_text = st.text_area("Paste Tickers", value="QQQ, SMH, NVDA, XLF", height=100)
    csv_file = st.file_uploader("Or Upload CSV Watchlist", type=["csv"])  # ✅ CSV UPLOAD ADDED
...
if not run and not st.session_state.analysis_ran: st.stop()
if not key or not secret:
    st.warning("Enter Alpaca credentials.")
    st.stop()

# Parse symbols from paste or CSV
symbols = []
if symbols_text.strip(): symbols.extend([s.strip().upper() for s in symbols_text.replace("\n", ",").split(",") if s.strip()])
if csv_file is not None: symbols.extend(parse_symbol_csv(csv_file))  # ✅ CSV PARSING
...
if run:
    ...
    # Try Alpaca first, fallback to Yahoo if SIP error
    try:
        daily_map = fetch_alpaca_daily_batch(symbols + ["SPY"], 5, key, secret, feed)
    except Exception as e:
        if "SIP" in str(e) or "subscription" in str(e).lower():
            st.warning("Alpaca SIP error — falling back to Yahoo Finance for all symbols")
            daily_map = {s: fetch_yahoo_fallback(s, "1D", 5) for s in symbols + ["SPY"]}
        else:
            st.error(f"Data fetch error: {e}")
            st.stop()
    
    if "SPY" not in daily_map or daily_map["SPY"].empty:
        st.error("Failed to fetch benchmark."); st.stop()
    
    rows, detail = [], {}
    progress = st.progress(0.0)
    
    for sym in symbols:
        progress.progress((symbols.index(sym) + 1) / len(symbols))
        daily_raw = daily_map.get(sym, pd.DataFrame())
        if daily_raw.empty: continue
        
        # Fetch Real 2H Data with fallback
        try:
            tactical_raw = fetch_alpaca_2hour(sym, 3, key, secret, feed)
            use_real = not tactical_raw.empty
        except:
            tactical_raw = fetch_yahoo_fallback(sym, "2H", 3)
            use_real = not tactical_raw.empty
        
        tactical_label = "2-Hour (Real Alpaca)" if use_real else "2-Hour (Yahoo Fallback)"
        if not use_real and tactical_raw.empty:
            tactical_raw = daily_raw.copy()  # Last resort fallback
        ...
        rows.append({
            ...
            "Weekly Call": "CALL" if weekly_raw.iloc[-1]["uo_gap"] > 0 else "PUT",
...
# Render results from session state (prevents reset on dropdown change)
...
# ✅ PERSISTENT DROPDOWN: Uses session_state to remember selection
...
st.session_state.selected_symbol = selected  # Persist selection
