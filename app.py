...
st.set_page_config(page_title="Stable Market Engine v8 | Context MC Ranked", layout="wide", initial_sidebar_state="expanded")
...
def fetch_alpaca_daily_batch(symbols: List[str], years: int, key: str, secret: str, feed: str) -> Dict[str, pd.DataFrame]:
    ...
    except Exception: return data_map
    for s in missing:
...
def fetch_alpaca_2hour(symbol: str, months: int, key: str, secret: str, feed: str) -> pd.DataFrame:
    ...
    for _, day_df in sub.groupby(sub.index.date):
        ...
        for start_ts, end_ts in ranges:
            ...
            row = pd.DataFrame({"Open": [chunk["Open"].iloc[0]], "High": [chunk["High"].max()], "Low": [chunk["Low"].min()], "Close": [chunk["Close"].iloc[-1]], "Volume": [chunk["Volume"].sum()]}, index=[start_ts])
            bins.append(row)
        if bins: pieces.append(pd.concat(bins))
    out = pd.concat(pieces).sort_index() if pieces else pd.DataFrame()
    out = normalize_ohlcv(out)
    if not out.empty: out.to_parquet(p)
    return out
...
def add_ultimate_oscillator(out: pd.DataFrame, timeframe_name: str) -> pd.DataFrame:
    ...
    decision_smooth_map = {"hourly_proxy": 2, "real_1hour": 2, "2hour_proxy": 2, "real_2hour": 2, "daily": 1, "weekly": 1}
    viz_smooth_map = {"hourly_proxy": 6, "real_1hour": 7, "2hour_proxy": 6, "real_2hour": 8, "daily": 3, "weekly": 2}
    ...
    out["uo_base"] = weights["tsi"]*tsi_n + weights["cci"]*cci_n + weights["bb"]*bb_n + weights["vwap"]*vwap_n + weights["adx"]*adx_n + weights["z"]*z_n
    if timeframe_name in {"real_1hour", "real_2hour", "2hour_proxy", "hourly_proxy"}:
    ...
    out["uo"], out["uo_signal"] = out["uo_viz"], out["uo_signal_viz"]
    out["uo_gap"] = out["uo_decision"] - out["uo_signal_decision"]
    out["uo_slope_1"], out["uo_slope_3"] = out["uo_decision"].diff(1), out["uo_decision"].diff(3)
...
def enrich_price_features(df: pd.DataFrame, timeframe_name: str, benchmark_df: Optional[pd.DataFrame] = None) -> pd.DataFrame:
    ...
    if timeframe_name == "hourly_proxy": out["vwap"] = session_vwap(out)
    elif timeframe_name == "real_2hour": out["vwap"] = rolling_vwap(out, 12)
    elif timeframe_name == "2hour_proxy": out["vwap"] = rolling_vwap(out, 10)
    else: out["vwap"] = rolling_vwap(out, 20)
    ...
    out["price_change_while_tsi_flat"] = np.where(out["tsi_slope_3"].abs() <= tsi_flat_thresh, out["Close"].pct_change(3), 0.0)
    out["pinning_up_flag"] = ((out["price_change_while_tsi_flat"] > 0.012) & (out["cci_slope_3"] < 0) & (out["pct_b"] > 0.75)).astype(float)
    out["pinning_down_flag"] = ((out["price_change_while_tsi_flat"] < -0.012) & (out["cci_slope_3"] > 0) & (out["pct_b"] < 0.25)).astype(float)
    ...
    for col in ["rsi_14", "cci_20", "tsi", "pct_b", "atr_stretch", "adx_14", "dist_ema20_pct", "dist_vwap_pct", "close_zscore"]:
...
# -----------------------------
# v8 UPGRADES: Regime, Cross-Ticker Pool, State-Aware MC, Confidence
# -----------------------------
...
def regime_alignment_bonus(regime: str, signal: str) -> float:
    bonus = 0.0
    if regime == "BULL_TREND" and signal.startswith("CALL"): bonus += 0.05
    elif regime == "BEAR_DOWNTREND" and signal.startswith("PUT"): bonus += 0.05
    elif regime == "VOL_SPIKE": bonus -= 0.03
    return bonus

def build_analog_pool_batch(symbol: str, enriched_df: pd.DataFrame) -> pd.DataFrame:
    cols = ["Symbol", "Timestamp"] + ANALOG_FEATURES + ["fwd_ret_1", "fwd_ret_2", "fwd_ret_5"]
    df = enriched_df[ANALOG_FEATURES + ["fwd_ret_1", "fwd_ret_2", "fwd_ret_5"]].copy()
    df["Symbol"] = symbol
    df["Timestamp"] = df.index
    return df[[c for c in cols if c in df.columns]]

def search_cross_ticker_analogs(pool: pd.DataFrame, current_vec: pd.Series, symbol: str, n: int = 30) -> pd.DataFrame:
    if pool.empty or len(pool) < 20: return pd.DataFrame()
    use_cols = [c for c in ANALOG_FEATURES if c in pool.columns]
...
def compute_state_aware_mc(analog_df: pd.DataFrame, fallback_close: pd.Series, n_sim: int = 5000, horizons: list[int] = [1,2,3,4,5]) -> dict:
    ...
    if len(analog_df) >= 15:
        # State-aware bootstrapping
        for h in horizons:
            col = f"fwd_ret_{h}"
            if col not in analog_df.columns: 
                out[f"mc_up_{h}d"] = 0.5; continue
...
def classify_timeframe_call(row: pd.Series, timeframe: str) -> Tuple[str, str]:
    ...
    if is_structural:
        ...
        if uo_pct > 0.85 and below2 and uo_slope3 < 0 and gap_strength >= 0.18 and rsi_val > 65: return "PUT", "Confirmed daily rollover from elevated zone"
        if uo_pct < 0.18 and above2 and uo_slope3 > 0 and gap_strength >= 0.18 and rsi_val < 40: return "CALL", "Confirmed turn up from washed-out zone"
        if above2 and uo_slope3 > 0 and gap_strength >= 0.15: return "CALL", "Confirmed composite above signal"
        if below2 and uo_slope3 < 0 and gap_strength >= 0.15: return "PUT", "Confirmed composite below signal"
        return "NEUTRAL", "No confirmed structural cross"
    if uo_gap > 0 and uo_slope3 > 0: return "CALL", "Composite rising above signal"
    if uo_gap < 0 and uo_slope3 < 0: return "PUT", "Composite below signal and falling"
...
def compute_severity(frame: pd.DataFrame, row: pd.Series) -> Dict[str, float]:
    ...
    return {"bars_above_90": bars_above_90, "bars_below_10": bars_below_10, "late_cycle_flag": int(bars_above_90 >= 4 and float(row.get("uo_slope_3", 0.0)) <= 0)}

def recommendation(call: str, severity: Dict[str, float], analogs: Dict[str, float]) -> str:
    if call == "CALL":
        if severity.get("late_cycle_flag", 0): return "CALL, but extended"
        if analogs and analogs.get("ret_2_p_up", 0.5) > 0.62: return "CALL / analogs supportive"
        return "CALL"
    if call == "PUT":
        if analogs and analogs.get("ret_2_p_down", 0.5) > 0.62: return "PUT / analogs confirm"
        return "PUT setup forming"
    if severity.get("late_cycle_flag", 0): return "WAIT, aging trend"
...
def combine(proxy_call: str, daily_call: str, weekly_call: str) -> str:
    ...
    if net >= 0.55: return "CALL"
    if net <= -0.55: return "PUT"
    if daily_call == "CALL" and weekly_call == "CALL": return "CALL ON PULLBACK"
    return "NEUTRAL"

def align_asof(index: pd.DatetimeIndex, dt: pd.Timestamp) -> pd.Timestamp:
    ts = pd.Timestamp(dt)
    if getattr(index, "tz", None) is not None and ts.tzinfo is None: return ts.tz_localize(index.tz)
    if getattr(index, "tz", None) is None and ts.tzinfo is not None: return ts.tz_localize(None)
    return ts
...
def distance_to_cross(row: pd.Series, frame: pd.DataFrame) -> Dict[str, float]:
    if row is None or row.empty or frame.empty: return {"gap": np.nan, "abs_gap": np.nan, "range_pct": np.nan}
...
def frame_warning_message(row: pd.Series, timeframe_label: str, structural_bias: str = " ") -> str:
    if row is None or row.empty: return f"{timeframe_label}: no data."
    uo_gap = float(row.get("uo_gap", 0.0))
    uo_pct = float(row.get("uo_pctile", 0.5))
    ...
    if bullish and price_slope <= 0: return f"{timeframe_label}: oversold and improving, but price has not lifted yet — early bullish divergence only."
    if bullish and price_slope > 0: return f"{timeframe_label}: washed-out rebound is starting to confirm with price improvement."
    if bearish and price_slope >= 0: return f"{timeframe_label}: exhausted/rolling, but price has not broken yet — be careful pressing shorts."
    if bearish and price_slope < 0: return f"{timeframe_label}: exhaustion is starting to confirm with price weakness."
    if abs(tsi_slope) < 0.05 and cci_slope < 0 and price_slope >= 0: return f"{timeframe_label}: momentum fading under surface while price holds — caution."
    if abs(tsi_slope) < 0.05 and cci_slope > 0 and price_slope <= 0: return f"{timeframe_label}: internal momentum improving while price lags — watch upside."
    if structural_bias: return f"{timeframe_label}: aligned with broader {structural_bias.lower()} bias, no special warning."
    return f"{timeframe_label}: mixed state."

def traffic_state(call: str, row: pd.Series) -> Tuple[str, str]:
    if row is None or row.empty: return "⚪", "No data"
    pct = float(row.get("uo_pctile", 0.5))
    gap = float(row.get("uo_gap", 0.0))
    slope3 = float(row.get("uo_slope_3", 0.0))
    if call == "CALL":
        return ("🟢", "Bull turn") if (pct < 0.20 and gap > 0 and slope3 > 0) else ("🟢", "Bullish")
    if call == "PUT":
        return ("🔴", "Bear roll") if (pct > 0.80 and gap < 0 and slope3 < 0) else ("🔴", "Bearish")
    return ("🟡", "Extreme") if (pct > 0.80 or pct < 0.20) else ("🟡", "Neutral")
...
def plot_dashboard(symbol: str, hourly_df: pd.DataFrame, tactical_df: pd.DataFrame, daily_df: pd.DataFrame, weekly_df: pd.DataFrame, asof_date: pd.Timestamp, tactical_label: str) -> None:
    fig = make_subplots(rows=5, cols=1, vertical_spacing=0.04,
                        subplot_titles=[f"{symbol} Daily Price (as of {pd.Timestamp(asof_date).date()})", "1-Hour UO", f"{tactical_label} UO", "Daily UO", "Weekly UO"],
...
# Main App
# -----------------------------
st.title("📈 Stable Market Engine v8 | Context MC Ranked")
...
with st.sidebar:
    ...
    st.header("📥 Input")
    symbols_text = st.text_area("Paste tickers (comma/line separated)", value="QQQ, SMH, INTC, NVDA, AMD, XLF", height=80)
    upload_watchlist = st.file_uploader("Upload watchlist CSV", type=["csv"])
    st.header("⚙️ Settings")
    benchmark = st.selectbox("Benchmark", ["SPY", "QQQ", "RSP", "IWM"], index=0)
    history_years = st.selectbox("History (Years)", [3, 5, 10], index=1)
    analysis_mode = st.radio("Analysis Mode", ["Current", "Historical"], index=0)
    analysis_date = st.date_input("Lookback Date", value=pd.Timestamp.today().date(), disabled=(analysis_mode == "Current"))
...
# v8: Classify Regime Once
regime = classify_market_regime(benchmark_daily)
st.info(f"🌍 Market Regime: `{regime}` (Trend Slope & Vol Ratio)")

# v8: In-Memory Cross-Ticker Pool
pool_batch_list = []
...
for i, sym in enumerate(symbols):
    ...
    # v8: Add to cross-ticker pool
    daily_fwd = add_forward_returns(daily_df)
    pool_batch_list.append(build_analog_pool_batch(sym, daily_fwd))

progress.empty()
global_analog_pool = pd.concat(pool_batch_list, ignore_index=True) if pool_batch_list else pd.DataFrame()

# Second Pass: Rank & Score
...
for i, sym in enumerate(symbols):
    ...
    # Signals
    hourly_call, hourly_reason = classify_timeframe_call(hourly_row, hourly_timeframe if hourly_view.empty else cache["hourly_label"].lower().replace(" (proxy)", "_proxy").replace(" ", ""))
    ...
    # v8: Cross-Ticker Analog Search
    current_vec = daily_row[ANALOG_FEATURES].fillna(0) if len(daily_row) > 0 else pd.Series(dtype=float)
    analogs = search_cross_ticker_analogs(global_analog_pool, current_vec, sym, n=30)
    analog_summary = {}
    ...
    # v8: State-Aware Monte Carlo & Confidence
    ...
    conf_grade, conf_weight = compute_confidence_grade(int(analog_summary.get("n", 0)), avg_sim, regime, combined, vol_ratio)
    ...
    detail[sym] = {
        ...
        "mc_probs": mc_probs, "conf_grade": conf_grade, "regime": regime
    }
...
st.download_button("⬇️ Download results CSV", results_df.to_csv(index=False).encode("utf-8"), "v8_mc_ranked_results.csv", "text/csv")
